# backend/import_processor/nodes/node_item_name_recognition.py
import json
...
class NodeItemNameRecognition(BaseNode):
    ...
    def _step_3_call_llm(self, file_title: str, context: str) -> str:
        ...
        try:
            ...
            # （如 api.openai-proxy.org）收到该参数会直接返回 400 Unrecognized request argument，
            # 因此仅在显式配置 LLM_ENABLE_THINKING 时才下发。
            llm_kwargs = {}
            ...
            # 6. 数据清洗：按空白符切分后再用单个空格拼接
            # 等价于「去掉首尾空白 + 把换行/Tab/连续空格压缩成单个空格」
            # 不可用 replace(" ", "")：那会把品牌名内部的规范空格一并删掉，
            # 例如 "H3C LA2608室内无线网关" 会被破坏为 "H3CLA2608室内无线网关"
...
if __name__ == "__main__":
    ...
    # 调试目标：指向 out/<文件标题>/chunks.json（由 node_document_split 产出）
    chunks_path = str(
        _backend_dir / "out" / "H3C LA2608室内无线网关 用户手册-6W100-整本手册" / "chunks.json")
    ...
    init_state = {
        "chunks": chunks,
        # file_title 由产物目录名推导，避免与 chunks 内容不一致
        "file_title": Path(chunks_path).parent.name,
    }
