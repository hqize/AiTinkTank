# backend/import_processor/nodes/node_pdf_to_md.py
import json
import logging
import shutil
import time
...
if __name__ == "__main__":
    ...
    # 单独调试本节点时，需直接提供 process() 所依赖的 pdf_path 与 file_dir
    # （正常流程中 pdf_path 由上游 node_entry 写入，file_dir 为输出目录）
    _backend_dir = Path(__file__).resolve().parents[3]

    init_state = {
        "pdf_path": str(_backend_dir / "doc" / "hak180产品安全手册.pdf"),
        "file_dir": str(_backend_dir / "out"),
    }

    # 使用  "对象()"的方式调用对象的 __call__方法
    result = node_pdf_to_md(init_state)
    # 将返回的图状态进行json序列化
    json_state = json.dumps(result, ensure_ascii=False, indent=4)
    # 输出
    node_pdf_to_md.logger.info(json_state)
