...
class NodeDocumentSplit(BaseNode):
    ...
    def _step_6_backup(self, state: ImportGraphState, sections: List[Dict[str, str]]) -> None:
        """
            ...
            :param state: 项目状态字典，读取md_path（备份目录取其所在目录）与file_title（兜底用）
            :param sections: 最终处理后的Chunk列表
        """

        try:
            # 拼接备份目录：优先与MD产物同目录（即 out/<file_title>/），无md_path时兜底重建
            md_path = state.get("md_path")
            if md_path:
                backup_dir = Path(md_path).parent
            else:
                _backend_dir = Path(__file__).resolve().parents[3]
                backup_dir = _backend_dir / "out" / str(state.get("file_title") or "")

            # 备份目录可能尚未创建（例如单独调试本节点时），需先递归创建再写入
            backup_dir.mkdir(parents=True, exist_ok=True)

            # 固定文件名，便于查找
            backup_path = str(backup_dir / "chunks.json")
